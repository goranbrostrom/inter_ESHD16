...
<<setup, include = FALSE>>=
knitr::opts_chunk$set(echo = TRUE, cache = TRUE, fig.height = 4)
@ 
...
library(xtable)
library(haven)
ef <- as.data.frame(read_dta("Episodes_file.dta")) 
...
age.win <- c(15, 50) # Note this!!!!!!!!!! Used everywhere; change here.
...
<<ranef, fig.cap = "The (sorted) random effects from the mixed effexts Cox regression.", fig.height = 6>>=
plot(sort(ranef(fit)[[1]]), type = "l", col = "blue", 
