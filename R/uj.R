uj <-
function (x, caption = NULL, label = NULL, dr = NULL, digits = max(options()$digits - 
    4, 3), ...) 
{
    library(xtable)
    ...
    coef <- x$coef
    names(coef) <- sanitize(names(coef))
    se <- sqrt(diag(x$var))
    wald.p <- formatC(1 - pchisq((coef/se)^2, 1), digits = digits, 
        format = "f")
    ...
    for (term.no in 1:length(term.names)) {
        if (ord[term.no] == 1) {
            ...
            if (isF[covar.no]) {
                if (lp) {
                  lpindx <- lpindx + 1
                  cat(covar.names[covar.no], " & & & & &", lpval[lpindx], 
                    " \\\\ \n")
                }
                else {
                ...
                x$levels[[covar.no]] <- substring(x$levels[[covar.no]], 
                  1, 16)
                lb <- paste("\\em", x$levels[[covar.no]][1], 
                            sep = " ")
                ##lb <- sanitize(lb)
                cat("\\multicolumn{1}{r}{", lb, "}", " & ", formatC(x$w.means[[covar.no]][1], 
                  width = 8, digits = digits, format = "f"), 
                  " & ", noll, " & ", ett, " & ", "\\multicolumn{2}{c}{(reference)} \\\\ \n", 
                  sep = "")
                for (lev in 2:no.lev) {
                  index <- index + 1
                  lb <- paste("\\em", x$levels[[covar.no]][lev], 
                              sep = " ")
                  ##lb <- gsub("_", "\\_", lb)
                  cat("\\multicolumn{1}{r}{", lb, "}", " & ", 
                    formatC(x$w.means[[covar.no]][lev], width = 8, 
                      digits = digits, format = "f"), " & ", 
                    coef[index], " & ", e.coef[index], " & ", 
                    se[index])
                  if (lp) {
                    cat("\\\\ \n")
                  }
                ...
                index <- index + 1
                cat(covar.names[covar.no], " & ", formatC(x$w.means[[covar.no]], 
                  width = 8, digits = digits, format = "f"), 
                  " & ", coef[index], " & ", e.coef[index], " & ", 
                  se[index], " & ")
                if (lp) {
            ...
            for (i in 1:ord[term.no]) {
                if (isF[covar.no[i]]) {
                  niv[i] <- length(x$levels[[covar.no[i]]]) - 
                    1
                }
                else {
                  niv[i] <- 1
            ...
            for (index in stt:(stt + prod(niv) - 1)) {
                ...
                cat(formatC(" ", width = 2), formatC(substring(vn, 
                  1, 22), width = 22, flag = "-"), " &  & ", 
                  coef[index], " & ", e.coef[index], " & ", se[index], 
                  " & ", formatC(wald.p[index], digits = digits, 
                    width = digits + 2, format = "f"), "\\\\ \n")
            }
        }
    }
    cat("\\hline \n")
    ...
    cat(" \n\n")
    if (FALSE) {
        tmp <- cbind(coef, exp(coef), se, signif(1 - pchisq((coef/se)^2, 
            1), digits - 1))
    ...
    if (FALSE) {
        ...
        cat("\n")
        if (length(x$icc)) 
            cat("   number of clusters=", x$icc[1], "    ICC=", 
                format(x$icc[2:3]), "\n")
        invisible(x)
    }
}
