...
We modify the model by \emph{stratifying on gmDeaths} (with the categories \emph{"0", "1", "2+"}) and plot the result, see Figure~\ref{fig:weibstrat}.

<<weibstrat, fig.cap = "Weibull regression, stratified by gmDeaths.">>=
fit.ws <- phreg(Surv(exit - enter, event) ~ strata(gmDeaths) + childBirthdate + parity, data = ef)
##drws <- drop1(fit.ws, test = "Chisq")
##ltx(fit.ws, dr = drws, caption = "Stratified Weibull regression.", label = "tab:weibstrat")
plot(fit.ws, fn = "cum", col = c(1, 2, 4), lty = c(1, 2, 4), main = "", xlab = "Age")
@

A formal test rejects the hypothesis of proportional hazards (no surprise), and a further test of equality of the curves also rejects (ditto). This is in exact agreement with our conclusion regarding the Cox regression model (Figure~\ref{fig:relcumhaz}).


The corresponding Weibull hazard functions are shown in Figure~\ref{fig:whaz}. Their values at zero are infinite.

<<whaz, fig.cap = "The Weibull baseline hazard functions.">>=
plot(fit.ws, fn = "haz", col = c(1, 2, 4), lty = c(1, 2, 4), main = "", xlab = "Age (years).", ylim = c(0, 0.4), 
     printLegend = FALSE)
abline(h = 0)
abline(v = 0)
@
