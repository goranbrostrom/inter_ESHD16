...
is nevertheless included in the study until 1900.  The region was large, both in terms of
...
parishes are digitized by the Demographic Data Base at Umeå University
...
\item[gmIMR] Grandmother's infant mortality rate, that is, the number of deaths divided by the number of births. Used as an alternative to {\tt gmDeaths} in some analyses. It is also multiplied by 100 and thus measured on a scale ranging from 0 to 100.
...
\section{Descriptive statistics of the IDS extraction}
...
\subsection*{The covered time period}
...
\subsection*{Grandmothers, mothers and mother--sisters}
...
The standard models are common to the subgroups of this research project.
...
This section is specific for the Skellefteå project.
...
this is of practical significance? The simplest model is the easiest to interpret: If grandmother has no infant deaths, then the probablility for mother to have one is 38 per cent, otherwise it is 45 per cent.  
