...
<<setup, include=FALSE>>=
knitr::opts_chunk$set(echo = FALSE, comment = NA, fig.height = 3.5, cache = TRUE)
...
<<poisp, fig.cap = 'LRT p-values for the covariates. Dashed line is significance limit with  "multiple comparisons" correction. '>>=
pv <- dr[[5]][-1]
...
These calculations are made for a mother born January 1, 1826. 
...
fit.c <- coxreg(Surv(exit - enter, event) ~ gmDeaths + gmBirths + childBirthdate + parity, data = ef)
...
fit.str <- coxreg(Surv(exit - enter, event) ~ strata(G0_InfD_cat) + I(G0_Births - 7) + I(mBirthdate + enter) + parity, data = ef)
...
ef$childBirthdate <- ef$mBirthdate + ef$enter
fit.c2 <- coxreg(Surv(exit - enter, event) ~ gmIMR + childBirthdate + parity, data = ef)
...
##ef$childBirthdate <- ef$mBirthdate + ef$enter
fit.w <- phreg(Surv(exit - enter, event) ~ gmIMR + childBirthdate + parity, data = ef)
