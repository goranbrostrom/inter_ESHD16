...
case the explanatory variable \emph{grandmother's IMR} is replaced by
...
analysis. The difference is that instead of mother's birth date, the
infant's birth date is used (no big difference), and a new variable, \emph{mother's age at child birth}, is introduced. It is centered around age 32. Other covariates, \emph{parity}, \emph{infant's birth date}, etc. are omitted, since they do not contribute much to the model. 
...
covariate \emph{grandmother's No.\ of infant deaths}, the real effect on
her daughter's risk of experiencing infant deaths appears when the number
of deaths are two or larger. \emph{Grandmother's No.\ of births} is also
influential, as is \emph{mother's age} with its characteristic $U$-shaped
...
The estimated cumulative hazards functions of the three groups of
grandmother's No. of infant deaths are shown by plotting the
\emph{cumulative hazards} for the three groups
(Figure~\ref{fig:relcumhaz}). This figure is based on stratification on the
levels ``no deaths'', ``exactly one death'', and ``two ore more deaths''.
