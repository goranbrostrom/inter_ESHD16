...
<<setup, include=FALSE>>=
knitr::opts_chunk$set(echo = FALSE, comment = NA, fig.height = 3)
...
The yearly numbers of births and deaths for the \emph{mothers in the data set} are shown in Figure~\ref{fig:numbirth}.

<<numbirth, fig.cap="Number of births and infant deaths by year in the data set.">>=
byear <- floor(ef$M_birthdate + ef$enter) + 1840
x <- table(byear)
years <- names(x)
plot(years, as.numeric(x), type = "l", xlab = "Birth/Death year", ylab = "Frequency", col = "blue", ylim = c(0, 1500))
dyear <- with(ef[ef$event, ], floor(M_birthdate + exit)) + 1840
y <- table(dyear)
years <- names(y)
...
Corresponding numbers from {\sc Poplink} are shown in Figure~\ref{fig:poplink}.

<<poplink, fig.cap = "Number of births and infant deaths by year in Poplink.">>=
library(skum)
...
plot(years, as.numeric(x), type = "l", col = "blue", xlim = c(1742, 1902), ylab = "Frequency", xlab = "Birth/Death year", ylim = c(0, 1500))
...
The difference betwen the two cases is of course explained by the fact that in the present data file there are restrictions on which births to include: Mother and grandmother present, etc.
...
The result is presented in two steps: First, \emph{the statistical significance} is calculated and shown, in Figure~\ref{fig:poisp}.
The solid horizontal red line at 5\% is our (conventionally) chosen nominal limit for statistical significance. The dashed line is the limit that should be respected in honor of the \emph{mass significance} problem. The first (leftmost) covariate, \emph{G0InfD\_ cat}, is  clearly statistically significant, while \emph{G0\_Births} is just barely significant. \emph{M\_birthdate} is clearly out. 
...
<<poisp, fig.cap = 'LRT p-values for the covariates. Dashed line is significance limit with  "mass significance" correction.'>>=
pv <- dr[[5]][-1]
names(pv) <- c("G0InfD_cat", "G0_Births", "M_birtdate")
xp <- sort(pv)
n <- length(xp)
mplin <- (1:n)/n * 0.05
plot(1:3, xp, type = "h", axes = FALSE, xlab = "Covariates", ylab = "p-value")
points((1:3)[xp < mplin], xp[xp < mplin], col = "darkgreen", pch = 19, cex = 2)
points((1:3)[xp >= mplin], xp[xp >= mplin], col = "red", pch = 18, cex = 2)
...
<<poiseff, fig.cap = "Relative risk increase of infant mortality, compared to no grandmother infant deaths (left) and grandmother births compared to two births(right).">>=
...
x <- 2:20
y <- exp(fit.p$coef[1] + x * fit.p$coef[4])
y <- y / y[1]
plot(x, 100 * (y - 1), type = "h", xlab = "G0_Births", ylab = "Relative IMR (per cent)", col = "blue", 
     main = "", ylim = c(-41, 1))
points(2, 0, col = "red")
abline(h = 0)
@
...
<<relcumhaz, fig.cap = "Cumulative hazards by the number of grandmother's infant deaths.", fig.height = 5>>=
