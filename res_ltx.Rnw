...
infant's birthdate is used (no big difference), and a new variable, \emph{mother's age at child birth}, is introduced. It is centered around age 31. Other covariates, \emph{parity}, \emph{infant's birthdate}, etc. are omitted, since they do not contribute much to the model. 
...
<<effstcox, fig.cap = "Effects of included covariates, Cox regression.">>=
...
fit.str <- coxreg(Surv(exit - enter, event) ~ strata(gmDeaths) + gmBirths + mAge + mAge2, data = ef)
