...
approximately ten per cent of the families\footnote{Detta säger isolerat ingenting om klustring: Måste modifieras!}. Despite the fact that infant
...
We can conclude that it was really simple to utilize the IDS data base with the aid of Quaranta's (2016) Stata script. The only drawback was that the script required \emph{Stata 14}, which is quite expensive. Luckily, Stata provides the possibility to try the software for free during one month, and that was enough time to get the script running.

Once we had the data, the analyses and report (this one) writing was performed in \emph{RStudio} \citep{rstudio} with the aid of the {\bf R} package {\tt knitr} \citep{knitr,xie15}. An environment that supports truly reproducible statistical research. 
