...
\citep{as94}\footnote{Referensen Edvinsson et al 20xx saknas}. 
...
\citep{ndj93}. In the catechetical registers the clergy kept 
...
information, for example on occupation, was recorded.  From the 1970s\footnote{Stoppa in "onwards" här är möjligt, men överflödigt.}, these records have been digitized by the Demographic Data Base at Umeå University, and individual level data from more than one hundred\footnote{Ändrat från "$< 100$", dvs mindre än 100. Antar att avsikten var "större än"?} parishes are at present available for research in the databases POPUM and POPLINK (\url{http://www.cedar.umu.se/}). 
...
digitalized regions. During recent years, the database has been extended
with parish registers until the 1950s for the Skellefteå and Umeå
regions. Data until 1950 are available in the anonymized data base POPLINK with a generational depth of up to 15 \citep{wee}. The registration in POPLINK of the Skellefteå data starts in 1699 with birth records. the  in Catechetical registers are available from 1720. Unfortunately there are flaws in the 18th century data about infant deaths.  Children that died young are underreported in the 18th century catechetical registers, and the death registers are missing before 1815. Hence the analysis in this paper is restricted to the 19th century. It includes the reproductive history of women born 1826--50. The public IDS database is extracted from the POPLINK database, but only until 1900 due to legal requirements of protection of privacy.
...
ef_fixed <- ef
save(ef_fixed, file = "ef_fixed.rda")
rm(ef_fixed)
@
...
\citet{lq16}, has fulfilled its purposess well. The use of the script has
...
to recent scientific developments \citep{mel14}. In the
...
régime. This can relate to local patterns of childcare, for example to the
...
environment. Mothers from a high mortality region experienced a higher infant mortality and vice versa \citep{lgt04} .
...
that experience. While statistically significant, the effect size is moderate\footnote{Jag tycker att vi bör påpeka att trots att p-värden är mycket små, så är den uppmätta effekten måttlig.} . We applied different models, all plausible, and got almost
